...
void label_to_img(PathFile &paths, const std::string &filename, int xres, int yres, float diagonal,
...
toimg option:
    ...
    void align_check(int argc, char *argv[]) {
        ...
        for (int64_t i = 0; i < pathcount; ++i) {
            ...
            if (!(i % (1024 * 512))) {
                std::cout << "[" << (i / (float) pathcount) * 100.f << "%] \t" << i << " paths checked out of "
                          << pathcount << std::endl;
            }
        }
...
void filter_by_location(int argc, char *argv[]) {
    ...
    std::cout << "Matching paths passing sphere of center " << pos[0] << ", " << pos[1] << ", " << pos[2]
...
//  std::cout << "Number of paths matching : " << resultpaths.size() << std::endl;
    ...
    pbrt::Film *film = new pbrt::Film(pbrt::Point2i(1024, 1024),
                                      ...
                                      std::unique_ptr<pbrt::Filter>(new pbrt::TriangleFilter(pbrt::Vector2f(1.f, 1.f))),
                                      35.f, outputfile, 1.f);
...
void label_to_img(PathFile &paths, const std::string &filename, int xres, int yres, float diagonal,
    ...
    std::unique_ptr<pbrt::FilmTile> tile(
            film->GetFilmTile(pbrt::Bounds2i(pbrt::Point2i(0, 0), pbrt::Point2i(xres, yres))));
